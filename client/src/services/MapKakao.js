        ...
        if (status === kakao.maps.services.Status.OK) {
          ...
          result.forEach((item) => {
            ...
            el.onclick = () => {
              ...
              marker = displayMarkerAndInfowindow(
                map,
                latlng,
                `${item.place_name} (${item.address_name})`,
              );
        ...
        (result, status) => {
          if (status === kakao.maps.services.Status.OK) {
            ...
            const displayAddress = placeName
              ? `${placeName} (${address})` // placeName이 있으면 placeName과 address 모두 표시
              : address; // placeName이 없으면 address만 표시

            // displayMarkerAndInfowindow 함수를 사용하여 마커와 인포윈도우를 생성 및 표시
            marker = displayMarkerAndInfowindow(map, latlngs, displayAddress);
